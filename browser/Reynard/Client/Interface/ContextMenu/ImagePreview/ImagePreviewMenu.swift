    ...
    static func configuration(
        ...
        return UIContextMenuConfiguration(identifier: UUID().uuidString as NSString, previewProvider: previewProvider) { _ in
            UIMenu(title: "", children: [
                UIAction(title: AppText.text("Share Image"), image: UIImage(named: "reynard.square.and.arrow.up")) { _ in
                ...
                UIAction(title: AppText.text("Save to Photos"), image: UIImage(named: "reynard.square.and.arrow.down")) { _ in
                    loadImage(from: url) { image in
                        UIImageWriteToSavedPhotosAlbum(image, nil, nil, nil)
                    }
                },
                UIAction(title: AppText.text("Copy"), image: UIImage(named: "reynard.document.on.document")) { _ in
                    loadImage(from: url) { image in
