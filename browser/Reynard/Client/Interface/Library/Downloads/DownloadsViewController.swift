...
final class DownloadsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UIGestureRecognizerDelegate {
    ...
    private lazy var searchBar: UISearchBar = {
        ...
        searchBar.placeholder = AppText.text("Search Downloads")
    ...
    private let emptyStateView = SidebarEmptyBackgroundView(message: AppText.text("Files you download appear here"))
    ...
    fileprivate func makeDownloadsMenu() -> UIMenu {
        UIMenu(title: "", children: [
            UIAction(title: AppText.text("Open Downloads Folder"), image: UIImage(named: "reynard.folder")) { [weak self] _ in
                self?.openDownloadsFolder()
            },
            UIAction(title: AppText.text("Clear Downloads History"), image: UIImage(named: "reynard.arrow.down.circle.badge.xmark")) { [weak self] _ in
    ...
    private func updateEmptyState() {
        emptyStateView.message = AppText.text(query.isEmpty ? "Files you download appear here" : "No matching downloads")
    ...
    func tableView(
        ...
        switch item.state {
        case .downloading:
            let cancelAction = UIContextualAction(style: .destructive, title: AppText.text("Cancel")) { [weak self] _, _, completion in
        ...
        case .completed:
            let deleteAction = UIContextualAction(style: .destructive, title: AppText.text("Delete")) { _, _, completion in
            ...
            let shareAction = UIContextualAction(style: .normal, title: AppText.text("Share")) { [weak self] _, _, completion in
                guard let self else {
                    completion(false)
                    return
                }
            ...
            let openAction = UIContextualAction(style: .normal, title: AppText.text("Open in\nFiles")) { [weak self] _, _, completion in
                guard let self else {
                    completion(false)
                    return
                }
    ...
    private func confirmCancelDownload(
        ...
        AlertPresenter.show(
            title: AppText.text("Cancel Download?"),
            message: AppText.format("Do you want to stop downloading %@?", item.fileName),
            buttons: [
                AlertPresenter.Button(title: AppText.text("Keep Downloading"), style: .cancel) {
                    completion(false)
                },
                AlertPresenter.Button(title: AppText.text("Cancel Download"), style: .destructive) {
