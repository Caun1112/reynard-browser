...
final class AddonPermissionsPreferencesViewController: SettingsTableViewController {
    ...
    private var permissionSections: [SectionModel] {
        ...
        if !requiredPermissions.isEmpty {
            sections.append(
                SectionModel(
                    headerTitle: AppText.text("Required Permissions"),
        ...
        if !optionalRows.isEmpty {
            sections.append(SectionModel(headerTitle: AppText.text("Optional Permissions"), displayedRows: optionalRows))
        }
        ...
        if let requiredDataCollectionDescription = AddonPermissionSupport.requiredDataCollectionDescription(for: metaData.requiredDataCollectionPermissions) {
            sections.append(
                SectionModel(
                    headerTitle: AppText.text("Required Data Collection"),
        ...
        if !optionalDataCollectionPermissions.isEmpty {
            sections.append(
                SectionModel(
                    headerTitle: AppText.text("Optional Data Collection"),
    ...
    init(addonID: String) {
        ...
        title = AppText.text("Permissions")
    }
        ...
        Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.addon = addon
                    self.tableView.reloadData()
                    AlertPresenter.show(title: AppText.text("Failed to update permissions"), message: "\(error)")
                }
            }
        }
    }
