...
final class JITSettingsSection: NSObject {
    ...
    func cell(at index: Int, tintColor: UIColor?) -> UITableViewCell {
        ...
        case .enableJIT:
            ...
            cell.textLabel?.text = AppText.text("Enable JIT")
            cell.selectionStyle = .none
            cell.accessoryView = jitSwitch
            return cell
        case .importPairingFile:
            let cell = SettingsViewUtils.actionCell(title: AppText.text("Import Pairing File..."), tintColor: tintColor)
    ...
    func savePairingFile(from url: URL) {
        backgroundQueue.async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    AlertPresenter.show(title: AppText.text("Import Failed"), message: error.localizedDescription)
                }
            }
        }
    }
    ...
    private func confirmDDIDownload(for sender: UISwitch) {
        ...
        let alert = UIAlertController(
            title: AppText.text("Preparing JIT"),
            message: AppText.text("Since this is your first time enabling JIT, Reynard needs to download and mount the Developer Disk Image. This is required for JIT to work properly."),
        ...
        DDIManager.shared.ensureRequiredDDIFiles(
            ...
            completion: { [weak self] result in
                ...
                case .failure(let error):
                    ...
                    SettingsViewUtils.dismissPresentedAlert(alert, from: settingsController) {
                        AlertPresenter.show(title: AppText.text("Download Failed"), message: error.localizedDescription)
                    }
                }
            }
        )
    ...
    private func showRestartAlert() {
        let alert = UIAlertController(
            title: AppText.text("Restart Required"),
            message: AppText.text("The app will now close for the JIT setting to take effect."),
            preferredStyle: .alert
        )
    ...
    private func performanceDetailLabel() -> UILabel {
        ...
        detailLabel.text = AppText.text("Enable JIT to improve performance and ensure websites work properly.")
        return detailLabel
    }
    ...
    private func unsupportedVersionWarningLabel() -> UILabel {
        ...
        warningLabel.text = AppText.text("JIT is not supported on the OS version you are using.")
        if #available(iOS 17.0, *) {
            if #unavailable(iOS 17.0.1) {
                warningLabel.text = AppText.text("Install the TrollStore version of Reynard to enable JIT automatically for improved performance and to ensure websites work properly.")
            }
        }
        return warningLabel
    }
