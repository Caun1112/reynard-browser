...
final class UpdatesSettingsSection {
    ...
    func cell(at index: Int, allowUpdate: Bool, tintColor: UIColor?) -> UITableViewCell {
        ...
        case .updateNow:
            let cell = SettingsViewUtils.actionCell(title: AppText.text("Update Now"), tintColor: tintColor)
    ...
    func trollStoreFooterView() -> UIView {
        return footerView(text: AppText.text("Make sure TrollStore's URL Scheme is enabled."))
    }
    
    func unsupportedUpdatesFooterView() -> UIView {
        return footerView(
            text: AppText.text("This build does not support in-app updates. Visit the project's Releases page to download the latest version of the app.")
        )
    }
    ...
    private func beginUpdate(from viewController: UIViewController) {
        guard let updateFeedData = BrowserUpdates.shared.sourceData,
            ...
            AlertPresenter.show(title: AppText.text("Update Unavailable"), message: AppText.text("Could not retrieve the download URL."))
            return
        }
        ...
        if installedThroughTrollStore {
            ...
            downloadUpdate(
                ...
                message: AppText.text("When the download finishes, choose the app that you used to sideload Reynard in the share sheet to install the update."),
    ...
    private func downloadUpdate(
        ...
        let alert = UIAlertController(title: AppText.text("Downloading Update"), message: message, preferredStyle: .alert)
