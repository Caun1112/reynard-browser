...
final class SelectPicker {
    ...
    private func showSingleSelectFallback(in geckoView: UIView) {
        ...
        let alert = UIAlertController(title: AppText.text("Select Option"), message: nil, preferredStyle: .actionSheet)
        for item in selectableChoices(from: choices) {
            let title = item.label.isEmpty ? AppText.text("Option") : item.label
